...
- [ ] **A3b.6 — Stage soak + promote.** **In progress (infra up):** `xo-training-staging` Fly app created + deployed from `backend/fly.training.toml` (2× shared-cpu/2/1gb machines, iad region, mirrors staging-backend secrets: DATABASE_URL → xo-db-staging.flycast, REDIS_URL → Upstash, INTERNAL_SECRET). `ml.useWorker=true` set in staging SystemConfig; staging backend restarted so signalBus subscribes to Redis and `trainingHealthMonitor` boots. Worker boot logs confirm `concurrency=2 jobsPerSecond=4` (A3b.3 defaults) + `training worker ready`. **Next:** 1-hour 10-virtual-user × mixed-algo soak via new `backend/src/scripts/trainingWorkerSoak.js` (drives the QA endpoints, polls `/admin/health/training` every 30s, writes a JSON report). QA endpoint extended: `POST /qa/training-worker/start` now accepts `algorithm` body param (defaults `qlearning`) and `_qaEnsureSeedSkill(slot, algorithm)` mints per-(algorithm, slot) skill rows so the soak can fan out without different algos clobbering each other's BotSkill. Pending: actually run the soak; capture report; if green, you invoke `/promote`.
