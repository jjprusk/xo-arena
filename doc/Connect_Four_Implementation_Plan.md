...
| A3a — Training data + UX rework (in-process) | in progress | 11/15 | Corpus: four new training docs (presets+time, multi-curve eval, pause/resume, self-play vs eval). Earlier: pause/resume, checkpoint recovery, multi-curve eval, per-user cap, ETA gate, presets, schema audit, channel rename, tournament ELO + series-winner attribution. |
