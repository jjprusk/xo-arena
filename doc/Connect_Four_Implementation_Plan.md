...
| A3a — Training data + UX rework (in-process) | in progress | 10/15 | Pause/Resume backend (force-checkpoint on pause; resume restarts loop from last checkpoint). Earlier: checkpoint recovery, multi-curve eval, per-user cap, ETA gate, presets, schema audit, channel rename, tournament ELO + series-winner attribution. |
