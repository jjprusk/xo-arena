...
| A3b — Training worker process cutover + training UI | in progress | 2/10 | Sequenced as 10 PR-sized chunks. **Shipped:** A3b.1 spike (BullMQ + xo-training worker round-trip + Dockerfile lockfile fix); A3b.2a (worker can run the real training loop via `training:start` jobs in shadow mode — `_runTrainingForQueueJob` + QA endpoint + 4 unit tests; backend `setImmediate` path is unchanged). **Next:** A3b.2b (flag-gated cutover — startTraining enqueues instead of setImmediate). |
