...
| A3b — Training worker process cutover + training UI | not started | 0/12 | New `xo-training` Fly app, Redis queue, pub/sub streaming, plus the deferred A3a training-UI work. |
...
- [ ] **Stacked W/D/L visualization (moved from A3a).** Each eval point emits W/D/L tallies via the A3a multi-curve eval module; frontend renders a stacked area chart per opponent curve. Saturated curves (e.g., 100% wins against Easy) auto-fade. Lands once the worker cutover stabilises the realtime stream.
- [ ] **Master-vs-bot split (moved from A3a; lights up in Phase B).** If eval includes Master, split into "as first-mover" and "as second-mover" sub-charts. No-op for TTT (no Master tier) — wires up for real when C4 ships its Master solver.
- [ ] **Training UI "no knobs" disclosure (moved from A3a).** v1 surface keeps presets only (Quick / Standard / Deep); advanced controls (custom iterations, learning rate, eval budget overrides) live behind an "Advanced" disclosure that only appears for admins or via a per-user feature flag.
