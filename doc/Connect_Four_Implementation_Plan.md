...
| A3b — Training worker process cutover + training UI | in progress | 1/10 | Sequenced as 10 PR-sized chunks. **Shipped:** A3b.1 spike — BullMQ + new xo-training worker round-trip working end-to-end locally; Dockerfile fix so workspace installs pick up bullmq's transitives. **Next:** A3b.2a (worker consumes `training:start` in shadow mode). |
...
- [ ] **A3b.2a — Worker can consume `training:start` end-to-end (shadow mode).** Worker process boots, listens to a `training:start` job on the queue, and runs the existing `_runTraining` against a non-prod test skill (the same `qa-recovery-seed` fixtures the harness already uses). Backend continues to run the loop in-process for real traffic — this PR only proves the worker side. Tests: worker happy path on test fixture; pause/resume + checkpoint write still land at the same per-episode cadence.
- [ ] **A3b.2b — Cut over: backend enqueues instead of `setImmediate`.** Single flag flip behind a `ml.useWorker` SystemConfig kill switch for instant rollback. `mlService.startTraining` switches from `setImmediate(() => _runTraining(...))` to `queue.add('training:start', ...)`. Pause / resume / cancel signals route via Redis pub-sub. Crash recovery: BullMQ's job lock + heartbeat replaces `resumeOrphanedSessions()`'s boot-time scan. SSE `training:<sessionId>` channel: worker publishes progress events to Redis, backend subscribes and fans out (clients see no contract change). The 3-session parallel crash-recovery harness now exercises the worker path. **Highest-risk PR in A3b** — feature-flag gated so a single SystemConfig flip reverts without a deploy.
