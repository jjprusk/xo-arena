...
| A3a — Training data + UX rework (in-process) | in progress | 4/15 | TrainingSession audit + new TrainingCheckpoint / TrainingMetric tables; channel rename `ml:session:*` → `training:*` across backend + landing. Tournament ELO + series-winner attribution shipped earlier in the sprint. |
