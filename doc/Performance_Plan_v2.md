...
| **20.3b** | TablesPage SWR retrofit (deferred from 20.3) | 0.5 day | low | Lower-value than 20.3 — Tables is authed + SSE-driven so the cached-paint win is small | Needs `mutate(updater)` added to `useSWRish` so the optimistic prepend on create still works. Land alongside if a Tables-specific perf complaint shows up; otherwise low priority. |
