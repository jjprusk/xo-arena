...
export async function getOpeningBook(modelId) {
  ...
  const emptyKey = '.........'
  const firstMoveQVals = weights[emptyKey] ?? Array(9).fill(0)
  ...
  for (let i = 0; i < 9; i++) {
    ...
    const qvals = weights[key] ?? Array(9).fill(0)
    responses.push({ opponentCell: i, qvals })
  }

  return { firstMoveQVals, responses, stateCount: Object.keys(weights).length }
}

// ─── Q-table / move ──────────────────────────────────────────────────────────

export async function getWeights(modelId) {
  const model = await db.botSkill.findUnique({ where: { id: modelId }, select: { weights: true } })
...
export async function getMoveForModel(modelId, board) {
  if (!engineCache.has(modelId)) {
    ...
    if (!model) throw new Error(`Bot skill ${modelId} not found`)
...
export async function updateElo(modelAId, modelBId, outcome) {
  ...
  // ELO is now stored in GameElo — look up the bot user for each skill.
  const [botA, botB] = await Promise.all([
    db.user.findFirst({ where: { botModelId: modelAId, isBot: true }, select: { id: true } }),
    db.user.findFirst({ where: { botModelId: modelBId, isBot: true }, select: { id: true } }),
  ])
  ...
  const rB = eloB?.rating ?? 1000
  const eA = _expectedScore(rA, rB)
  const eB = _expectedScore(rB, rA)
  const sA = outcome === 'WIN' ? 1 : outcome === 'DRAW' ? 0.5 : 0
  const sB = 1 - sA
  const newA = parseFloat((rA + ELO_K * (sA - eA)).toFixed(2))
  const newB = parseFloat((rB + ELO_K * (sB - eB)).toFixed(2))
...
export async function importModel(data) {
  const { name, description, algorithm, config, weights, qtable, totalEpisodes, createdBy = null } = data
  if (!name?.trim()) throw new Error('name is required')
  const mergedConfig = { ...DEFAULT_CONFIG, ...(config || {}) }
  const maxEpisodes = await getSystemConfig('ml.maxEpisodesPerModel', 100_000)
  // Accept 'qtable' from older exports for backward compatibility
  const weightData = weights || qtable || {}
  return db.botSkill.create({
    data: {
      name: name.trim(),
...
export async function finishTrainingFromFrontend(sessionId, { weights, stats, iterations, status = 'COMPLETED', samples }) {
  ...
  setImmediate(async () => {
    try {
      ...
      const startElo = calibGameElo?.rating ?? 1200
 ...
 * Accepts both legacy enum format ('Q_LEARNING') and new string format ('qlearning'). */
function _buildEngine(modelConfig, algorithm) {
  // Normalise: strip underscores, uppercase → handles both 'qlearning' and 'Q_LEARNING'
  const alg = (algorithm || 'qlearning').toUpperCase().replace(/_/g, '')
...
async function _runTraining(model, session, { mode, iterations, config }) {
  ...
  const algorithm = config.algorithm || model.algorithm || 'qlearning'
  ...
  const algNorm = algorithm.toUpperCase().replace(/_/g, '')
  if (algNorm === 'DQN') {
...
async function _finishSession(sessionId, modelId, engine, iterations, status, { wins, losses, draws, totalQDelta }, extraMeta = {}) {
  ...
  // ELO calibration: play 100 games vs each fixed minimax level and update GameElo
  try {
    const botUser = await db.user.findFirst({ where: { botModelId: modelId, isBot: true }, select: { id: true } })
    if (botUser) {
      const [calibGameElo, freshModel] = await Promise.all([
...
export async function ensembleMove(modelIds, method, weights, board, mark) {
  ...
  const engines = valid.map(m => {
    const alg = (m.algorithm || 'qlearning').toUpperCase().replace(/_/g, '')
  ...
  if (method === 'weighted' && weights && weights.length === valid.length) {
    ...
    const allTabular = engines.every(({ alg }) => !['DQN', 'ALPHAZERO', 'AZ'].includes(alg))
  ...
  const actions = engines.map(({ engine, alg }) => {
    ...
    if (alg === 'ALPHAZERO' || alg === 'AZ') return engine.chooseAction(board, mark)
    // Tabular: greedy
    engine.epsilon = 0
    return engine.chooseAction(board, false)
  })
