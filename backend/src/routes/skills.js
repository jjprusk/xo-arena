...
import * as svc from '../services/skillService.js'
...
router.get('/models/:id/weights', async (req, res, next) => {
  try {
    const weights = await svc.getWeights(req.params.id)
    res.json({ modelId: req.params.id, stateCount: Object.keys(weights).length, weights })
  } catch (err) { next(err) }
})

// Legacy alias — kept for older frontend versions
router.get('/models/:id/qtable', async (req, res, next) => {
  try {
    const weights = await svc.getWeights(req.params.id)
    res.json({ modelId: req.params.id, stateCount: Object.keys(weights).length, qtable: weights, weights })
  } catch (err) { next(err) }
})
