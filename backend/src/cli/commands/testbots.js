...
export async function seedTestBots(level = DEFAULT_LEVEL) {
  const results = []
  for (const bot of TEST_BOTS) {
    ...
    if (existing) {
      await db.user.update({ where: { id: existing.id }, data: { botModelId: modelId } })
      results.push({ ...bot, botModelId: modelId, status: 'updated', id: existing.id })
      continue
    }
    ...
    results.push({ ...bot, botModelId: modelId, status: 'created', id: created.id })
  }
  return results
}
  ...
  program
    ...
    .action(async (opts) => {
      try {
        ...
        const results = await seedTestBots(opts.level)
        for (const r of results) {
          if (r.status === 'created') ok(`Created ${r.displayName} @ ${r.botModelId} (${r.id})`)
          else console.log(`  → ${r.displayName} updated to ${r.botModelId} (${r.id})`)
        }
      } catch (err) {
        fail(err.message)
      }
